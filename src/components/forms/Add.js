...
const Add = ({Services}) => {
    ...
    const onPhotoChange = (event) => {
      ...
      if (event.target.files[0]) {
        if (event.target.files[0] !== 'image/jpeg' && event.target.files[0] !== 'image/png') {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
          ...
          if (error.response.data.error === "Name and Phone are required") {
            setErrorName(error.response.data.error.name);
            setErrorPhone(error.response.data.error.phone);
          } else if (error.response.data.error.name === "Name is required") {
            setErrorName(error.response.data.error.name);
            setErrorPhone(null);
            setError(null);
          } else if (error.response.data.error.phone === "Phone is required" || error.response.data.error === "Contacts validation failed: phone: Enter a valid phone number") {
            setError(null);
            setErrorName(null);
            setErrorPhone(error.response.data.error.phone ? error.response.data.error.phone : 'Enter a valid phone number')
          } else if (error.response.data.error === "Contact already exists") {
            ...
            setErrorName(null);
            setErrorPhone(null);
            setErrorEmail('Enter a valid email please')
          }
    ...
    return (
        <form className="form-container" onSubmit={handleSubmit}>
            <div className="form-field">
                ...
                {errorName ? <div className="error-message">
                        <p>{errorName}</p>
                      </div> : ''}
                <input onChange={onNameChange} className="form-input" type="text" id="name" name="name" value={name} placeholder="Name Lastname" />
            </div>

            <div className="form-field">
                <label className="form-label" htmlFor="phone">Phone:</label>
                {errorPhone ? <div className="error-message">
                        <p>{errorPhone}</p>
                      </div> : ''}
                <input onChange={onPhoneChange} className="form-input" type="tel" id="phone" name="phone" value={phone} placeholder="Phone" />
            </div>

            <div className="form-field">
                <label className="form-label" htmlFor="email">Email:</label>
                {errorEmail ? <div className="error-message">
                        <p>{errorEmail}</p>
                      </div> : ''}
                <input onChange={onEmailChange} className="form-input" type="text" id="email" name="email" value={email} placeholder="Email" />
            </div>

            <div className="form-field">
                <label className="form-label" htmlFor="company">Company:</label>
                <input onChange={onCompanyChange} className="form-input" type="text" id="company" name="company" value={company} placeholder="Company"/>
            </div>

            <div className="form-field">
                <label className="form-label" htmlFor="role">Role:</label>
                <input onChange={onRoleChange} className="form-input" type="text" id="role" name="role" value={role} placeholder="Role" />
            </div>

            <div className="form-field">
                <label className="form-label" htmlFor="city">City:</label>
                <input onChange={onCityChange} className="form-input" type="text" id="city" name="city" value={city} placeholder="City"/>
            </div>

            <div className="form-field">
                <label className="form-label" htmlFor="file-input">Upload a photo:</label>
                <input onChange={onPhotoChange} className="form-input" type="file" id="image" name="image" lang="en" />
                {errorPhoto ? <div className="error-message">
                        <p>{errorPhoto}</p>
                      </div> : ''}
            </div>
            ...
            <div className="form-field">
                ...
                <div className="form-icons">
                    <figure onClick={() => handleIcon('services')} className={handleIconName("services")}>{services}</figure>
                    <figure onClick={() => handleIcon('education')} className={handleIconName("education")}>{education}</figure>
                    <figure onClick={() => handleIcon('health')} className={handleIconName("health")}>{health}</figure>
                    <figure onClick={() => handleIcon('fairness')} className={ handleIconName("fairness")}>{fairness}</figure>
                </div>
            </div>
